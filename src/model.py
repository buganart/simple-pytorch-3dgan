...
class net_G(torch.nn.Module):
    def __init__(self, args):
        ...
        self.layerFinal = torch.nn.Sequential(
...
class net_D(torch.nn.Module):
    def __init__(self, args):
        ...
        self.layer1 = self.conv_layer(
            1, self.f_dim, kernel_size=4, stride=2, padding=(1, 1, 1), bias=self.bias
        )
        self.layer2 = self.conv_layer(
            self.f_dim,
            self.f_dim * 2,
    ...
    def forward(self, x):
        ...
        out = x.view(-1, 1, self.res, self.res, self.res)
        # print(out.size()) # torch.Size([32, 1, 32, 32, 32])
        out = self.layer1(out)
        # print(out.size())  # torch.Size([32, 32, 16, 16, 16])
        out = self.layer2(out)
